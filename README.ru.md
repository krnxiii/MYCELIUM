...
<p align="center">
  <a href="https://t.me/+93N9Kpw2PNcwYTEy"><img src="https://img.shields.io/badge/Telegram-Канал-blue?logo=telegram&style=flat-square" alt="Telegram" /></a>
</p>
...
- **Telegram-бот** — полный интерфейс к графу с телефона: текст, голос, фото, документы, пересылки. Быстрые команды + AI-агент
- **VPS-деплой** — граф на удалённом сервере 24/7: Tailscale VPN, Syncthing для синхронизации Vault, Telegram-бот
...
Установщик проведёт через 4 сценария:
...
| **4. VPS** (`make quickstart-vps`) | API или локальный | Граф 24/7, Telegram-бот, синхронизация Vault |
...
Полный интерфейс к графу через Telegram — текст, голосовые, фото, документы, пересылки.

**Два режима:**
- **Быстрый** — мгновенные команды без AI: `/capture`, `/search`, `/status`, `/today`, `/neurons`, `/domains`, `/abort`, `/level`
- **Полный** — свободный текст уходит тому же AI-агенту, что и в Claude Code, с доступом ко всем 33 MCP-инструментам

**Типы ввода:** текст, голос (Whisper локально или Deepgram), фото, документы (батчинг альбомов), пересылки с атрибуцией источника.

**Настройка:** создай бота через `@BotFather`, добавь токен в `.env` (`MYCELIUM_TELEGRAM__BOT_TOKEN`). VPS-сценарий настраивает это автоматически.
...
| `make quickstart-vps` | VPS-деплой (Neo4j + MCP + Telegram + Syncthing + Tailscale) |
| `make update` | Обновить код, пересобрать, перезапустить |
